...
export const createContractBodyWorkdaysPerWeekMin = 0.1;
...
export const CreateContractBody = zod.object({
  ...
  "workdaysPerWeek": zod.number().min(createContractBodyWorkdaysPerWeekMin).max(createContractBodyWorkdaysPerWeekMax).optional().describe('Arbeitstage pro Woche (0,1–7, Dezimalwerte erlaubt, z. B. 0,23 bei einem 24-h-Dienst pro Monat), Default 5.'),
...
export const updateContractBodyWorkdaysPerWeekMin = 0.1;
...
export const UpdateContractBody = zod.object({
  ...
  "workdaysPerWeek": zod.number().min(updateContractBodyWorkdaysPerWeekMin).max(updateContractBodyWorkdaysPerWeekMax).optional().describe('Arbeitstage pro Woche (0,1–7, Dezimalwerte erlaubt). Bei Angabe wird workdaysConfirmedAt gesetzt.'),
...
export const GetAllowanceSettingsResponse = zod.object({
  ...
  "vacationForecastEnabled": zod.boolean().describe('Optionale Hochrechnung des Urlaubsguthabens aus den bestätigten Arbeitszeiten der letzten 13 Wochen anzeigen? Konto-global, Standard AN. Die Prognose verändert den echten Kontostand nie.'),
...
export const UpdateAllowanceSettingsBody = zod.object({
  ...
  "vacationForecastEnabled": zod.boolean().optional().describe('Optionale 13-Wochen-Prognose anzeigen (konto-global).'),
...
export const GetVacationBalanceResponse = zod.object({
  ...
  "userId": zod.number(),
  "vacationDays": zod.number().describe('Aktueller Gesamtanspruch in Tagen inklusive erworbener Mehrarbeit.'),
  ...
  "vacationHoursTotal": zod.number().describe('Garantierter Sockel plus bereits erworbenes Mehrarbeitsguthaben.'),
  ...
  "dailyHoursSource": zod.enum(['contract', 'default']).optional().describe('Quelle der Tages-Stunden-Bewertung eines ganztägigen Urlaubstags zum heutigen Stichtag: Vertragsdaten (Wochenstunden ÷ Arbeitstage\/Woche) oder Standardwert. Bei \"contract\" sollte die UI auf die Datenpflege der Arbeitstage\/Woche hinweisen (Migrations-Default 5).'),
  ...
  "vacationSockelHours": zod.number().describe('Sockel des Urlaubstopfs (Urlaubswochen × Vertragsstunden), ohne Mehrarbeits-Aufbau.'),
  "vacationAufbauHours": zod.number().describe('Zusätzlicher Anspruch aus tatsächlich geleisteter Mehrarbeit über die Vertragsstunden des laufenden Kalenderjahres hinaus.'),
  "vacationForecastHours": zod.number().nullable().describe('Optional prognostizierter Jahresendstand auf Basis der letzten 13 Wochen; null bei ausgeschalteter oder noch nicht verfügbarer Prognose. Dieser Wert ist kein verfügbares Guthaben.'),
  "vacationForecastEnabled": zod.boolean().describe('Ist die optionale 13-Wochen-Prognose eingeschaltet?'),
...
export const GetHoursBalanceResponseItem = zod.object({
  ...
  "balance": zod.number(),
  "contractMonthlyTargetHours": zod.number().describe('Monatliches Vertrags-Soll (vertragliche Wochenstunden × 52\/12, fester Monats-Durchschnitt). 0 ohne (passenden) Vertrag. Planungs-Check fürs Dienstplan-Stundenkonto — unabhängig von balance.'),
  ...
  "vacationHoursUsed": zod.number().describe('Stundengenau verbrauchter Urlaub (spiegelt contract.vacationHoursUsed, 0 ohne Vertrag).'),
  "vacationHoursRemaining": zod.number().describe('Resturlaub in Stunden (vacationDays × vacationDailyHours − vacationHoursUsed).'),
  "vacationDailyHours": zod.number().describe('Typische Dienstlänge (Std\/Urlaubstag) dieser Bilanz-Zeile — Vertrag (Wochenstunden ÷ Arbeitstage\/Woche) oder Konto-Standardwert.'),
