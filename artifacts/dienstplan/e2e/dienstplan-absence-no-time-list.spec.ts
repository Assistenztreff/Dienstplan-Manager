...
/**
 * E2E-Test: Abwesenheiten (Urlaub/Krank) erscheinen auch in der LISTEN-/AGENDA-
 * Ansicht (view-toggle-list) als ganztägige Blöcke OHNE Uhrzeit, reguläre
 * Schichten dagegen MIT Uhrzeit.
 *
 * Die ShiftBadge-Komponente wird sowohl im Monatsgitter-Tagesdetail als auch in
 * der AgendaView gerendert. Der bestehende Test
 * (dienstplan-absence-no-time.spec.ts) deckt nur das Monatsgitter ab. Dieser
 * Test sichert dieselbe Darstellung in der Listenansicht gegen Regressionen ab:
 * Würde dort versehentlich wieder Start-/Endzeiten (z. B. "00:00") für
 * Urlaub/Krank eingeblendet, schlägt dieser Test fehl.
 *
 * Deckt ab:
 ...
 * - Umschalten auf die Listenansicht (view-toggle-list)
...
async function createAssistant(page: Page): Promise<{ id: number; name: string }> {
  ...
  const res = await page.request.post("/api/users", {
    data: {
      name: `E2E Abwesenheit Liste ${unique}`,
      email: `e2e.absence.list.${unique}@dienstplan.test`,
      role: "assistant",
    },
  });
...
test("Listenansicht: Urlaub/Krank zeigen nur den Typ, reguläre Schicht zeigt Uhrzeiten", async ({ page }) => {
