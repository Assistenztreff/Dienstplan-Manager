...
test("Warn-Toast erscheint beim Anlegen im abgeschlossenen Monat — aber nur einmal", async ({ page }) => {
  ...
  // Korrigiert 01.09.2026: Die Dienst-Zeilen liegen in der Wochen-Liste
  // (schedule-list -> agenda-view), NICHT im Monatsraster-Block
  // dienstplan-mobile. Die alte Einschraenkung konnte nie treffen.
  const badge = page.getByTestId("schedule-list").locator(`[data-testid^="shift-badge-"]`).first();
