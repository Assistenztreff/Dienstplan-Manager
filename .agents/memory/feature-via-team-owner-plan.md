...
- Same trap applies to NUMERIC limits (e.g. `historyMonths`/forward-planning), not just boolean features — any client-side pre-check via `getLimit(currentUser, ...)` silently misfires for non-owner planning-capable users once a role/access-level widening lets them reach that action. If the destination dialog/mutation already surfaces the server's structured 403 with a friendly message (e.g. `planUpgradeMessage`), the simplest correct fix is to skip the client pre-check entirely for non-owner users (gate the pre-check itself on `isAdmin`) rather than adding a new probe — the server (keyed off the team owner) remains authoritative either way.
