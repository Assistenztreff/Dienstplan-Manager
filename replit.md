...
- **Schichtmodelle statt Arbeitszeit-Vorlagen**: Es gibt KEINE separaten Arbeitszeit-Vorlagen mehr — das Schichtmodell (`shift_models`) trägt selbst `default_start_time`/`default_end_time` (HH:MM) + `default_weekdays` (int[] 1–7, Mo=1). Der Schicht-Dialog füllt Start/Ende aus dem gewählten Modell vor; bei Mehrfach-/Bulk-Anlage werden die Modell-Wochentage als Hinweis abgeglichen (`weekdayMismatchDates`), Anlegen an „unpassenden" Tagen bleibt bewusst erlaubt. „+ Neuen Dienst" ist für Free UND Premium sichtbar (Limit `maxShiftModels` greift erst beim Speichern).
- **Standard-Dienste-Seeding**: Registrierung legt 5 Modelle an (`default-shift-models.ts`) inkl. Standard-Start/Endzeit + Wochentagen. Urlaub/Krankheit sind KEINE Modelle — eigenes Abwesenheits-System.
- **Vergütungstyp pro Dienst** (`shift_models.compensation_type`: `regular` | `percentage` | `flat`): `regular` = Stundenlohn × IST-Stunden; `percentage` = Stundenlohn × IST-Stunden × `compensation_percent`/100; `flat` = fester Betrag `compensation_flat_cents` (unabhängig von Stunden). Steuert die Geld-Berechnung in der Lohnauswertung.
- **Zeiterfassung**: Ist-Zeiten (offen/bestätigt/abgelehnt). Schalter „Stundenzettel automatisch genehmigen" (`users.auto_approve_time_tracking`, admin-seitig): neu erfasste IST-Zeiten werden dann sofort als bestätigt gebucht. **Auswertungen**: Soll/Ist je Assistent & Monat.
- **Zuschläge STRIKT aus IST**: Zuschläge (Nacht/Sonntag/Feiertag) werden ausschließlich aus den **bestätigten IST-Stunden** (Zeiterfassung) berechnet, NICHT aus der Soll-Planung. Ohne bestätigte IST-Zeit kein Zuschlag.
- **Premium-Lohnauswertung** (`GET /dashboard/hours-balance`, Param `hourlyWage`): Geldwerte IMMER IST-basiert. Grundvergütung je Eintrag nach `compensationType` (s. o.); Zuschlags-Vergütung = Zuschlagsstunden × Stundenlohn auf die dynamischen IST-Sätze. `HoursBalanceRow` liefert `hourlyWage`, `basePay`, `night|sunday|holidaySurchargePay`, `totalPay` (Cent). **0%-Zuschläge werden NICHT aufgelistet** (UI `auswertungen.tsx` + PDF `pdf-export.ts` filtern `percent>0`).
...
- **Urlaub stundenbasiert** (Point 7): Urlaub wird stundengenau geführt — 1 Tag = 8 h (`hoursPerDay`); ein 24h-Dienst als Urlaub zählt 24/8 = **3,0 Tage**. `contracts.vacationHoursUsed` ist der maßgebliche Zähler; Berechnungsmethode `bwavg` (Durchschnitt) oder `factor`. `GET /contracts/:id/vacation-balance` liefert Stunden UND daraus abgeleitete Tage (`vacationDaysUsed = vacationHoursUsed / hoursPerDay`). UI zeigt überall **Tage = Stunden / 8** (Dashboard `AssistantVacationCard`, Resturlaub-Liste in `abwesenheiten.tsx`: bei Vertrag `vacationHoursUsed/8`, ohne Vertrag Fallback = geplante Urlaubs-Schichten dieses Jahres).
