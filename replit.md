...
- **Fehler-Tracking**: Zentraler Express-Error-Handler (`app.ts`, ganz am Ende) fängt unbehandelte Fehler, loggt, antwortet 500-JSON (ohne Details) und persistiert via `recordPlatformError` (`lib/platform-errors.ts`) in `platform_errors` (Level error|warning, Meldung, Kontext=Route, Zeitstempel; Aufbewahrung auf die 500 zuletzt AUFGETRETENEN begrenzt). **Bündelung**: gleiche Meldung + Kontext = EINE Zeile (manueller Update-then-Insert-Upsert, bewusst OHNE UNIQUE-Index — message bis 2000 Zeichen sprengt Btree-Limit); Wiederauftreten zählt `count` hoch, setzt `last_seen_at` UND `resolved=false` (Abhaken gilt für die Gruppe, neues Auftreten öffnet wieder). **Details/Stacktrace**: `platform_errors.last_stack` (nullable, gekürzt auf 4000 Zeichen) speichert den Detailtext des LETZTEN Auftretens (Upsert überschreibt immer, auch mit NULL); Error-Handler übergibt `err.stack`; UI: „Details"-Button je Zeile (nur wenn vorhanden) klappt `<pre>` mit dem Trace auf. Liste sortiert nach `lastSeenAt`; UI zeigt Badge „N×" + „zuletzt …". Helfer kann auch aus try/catch-Stellen gerufen werden. `GET /operator/errors?limit=` (Default 50, max 200, neueste zuerst, requireSuperadmin) antwortet als Objekt `{errors, totalStored, retentionLimit}` (`OperatorErrorList`; totalStored = Gesamtzahl gespeicherter Zeilen inkl. erledigter, retentionLimit = `MAX_STORED_ERRORS`) → Karte „Fehler-Tracking" (Leerzustand „Keine Fehler im Betrieb"). **Retention-Hinweis**: erreicht die Tabelle das Limit (`totalStored >= retentionLimit`), zeigt die Karte einen dezenten Hinweis (`text-error-retention-notice`, in Leerzustand UND Liste), dass älteste Einträge bereits entfernt wurden und die Liste unvollständig ist. **Abhaken**: `platform_errors.resolved` (boolean, Default false), `PATCH /operator/errors/:id` (`{resolved}`, requireSuperadmin, 404 bei unbekannter ID); Karte hat Filter „Offene" (Default, blendet erledigte aus) / „Alle" (erledigte ausgegraut + durchgestrichen, Button „Wieder öffnen"); sind alle erledigt, zeigt „Offene" einen eigenen Leerzustand. **Sammel-Abhaken**: Button „Alle abhaken" (nur sichtbar bei offenen Einträgen, mit Bestätigungs-Dialog) → `POST /operator/errors/resolve-all` (requireSuperadmin, setzt `resolved = true` für alle offenen, antwortet `{resolvedCount}`). Aufbewahrungslimit gilt unverändert auch für erledigte. Dev-Route `GET /api/dev/boom` (nur NODE_ENV≠production, `routes/health.ts`) löst absichtlich einen 500er aus. E2E: `dienstplan-operator-fehler-tracking.spec.ts` — deckt auch den Retention-Hinweis ab (unterhalb des Limits unsichtbar; am Limit sichtbar via Seed-Skript `pnpm --filter @workspace/scripts run seed-platform-errors` mit lauf-eindeutigem `SEED_PLATFORM_ERRORS_PREFIX`, `SEED_PLATFORM_ERRORS_COUNT` bzw. `SEED_PLATFORM_ERRORS_CLEANUP=1`; Cleanup restlos, zusätzlich crash-sicher im afterAll). **Aufbewahrungs-Regressionstest**: `prunePlatformErrors(limit)` exportiert, Limit via Env `PLATFORM_ERRORS_MAX_STORED` injizierbar (Modul-Load, Prod-Default 500); `platform-errors.retention.test.ts` läuft gegen die isolierte `_test`-DB (Env-Umbiegung VOR dynamischem Import, provisioniert sich bei Bedarf selbst via `setup-test-db`) und asserted: älteste nach `lastSeenAt` fliegen, neueste bleiben, erledigte zählen mit, Bündelung rettet Dauerbrenner.
