...
- **Durchgesetzt** (Plan des **Team-Eigentümers** ist maßgeblich, nicht des Anfragers):
  ...
  - `strictTimeTracking` (`PATCH /time-tracking/:id/confirm`) — Bestätigen/Ablehnen von Ist-Zeiten nur Premium. Free-Einträge bleiben „offen"; ACHTUNG: `dashboard/summary` zählt nur bestätigte Stunden als Ist-Stunden.
  - `calendarSync` (`GET /calendar-export`, `routes/calendar.ts`) — ICS-Export der FIX-Schichten (Assistenten: eigene; Admins: Team-Scope, optional `?teamId=`). Download-Card in `einstellungen.tsx` (`CalendarExportCard`).
