...
- **Urlaub stundenbasiert**: 1 Tag = 8 h (`hoursPerDay`); ein 24h-Urlaubsdienst zählt 24/8 = **3,0 Tage**. `contracts.vacationHoursUsed` ist der EINZIGE Zähler — die Alt-Spalte `vacation_days_used` wurde entfernt (Drop in `migrate-teams`, idempotent); Tage sind ÜBERALL abgeleitet (`vacationHoursUsed / vacationHoursPerDay`, gerundet auf 0,1: Lohnauswertung `vacationDaysUsed/Remaining`, Dashboard-Warnung `lowVacationAssistants`). Methode `bwavg` (Durchschnitt) oder `factor`. `GET /contracts/:id/vacation-balance` liefert Stunden UND abgeleitete Tage (`vacationDaysUsed = vacationHoursUsed / hoursPerDay`). UI zeigt überall **Tage = Stunden / 8** (Dashboard `AssistantVacationCard`; Resturlaub-Liste `abwesenheiten.tsx`: bei Vertrag `vacationHoursUsed/8`, ohne Vertrag Fallback = geplante Urlaubs-Schichten des Jahres). `vacationHoursUsed` MUSS im `CONTRACT_SELECT` der Route stehen, sonst rechnet die UI mit 0.
...
- **Backend-Scoping-Invarianten** (alle Domänen-Routen):
  ...
  - POST: Ziel-Team via `resolveWriteTeamId`; `time_tracking` erbt `teamId` von der Schicht. **Member-of-Team-Invariante**: `body.userId` muss Mitglied des Ziel-Teams sein, sonst 403 (PII-Leak) — gilt für POST contracts/time_tracking. **Ausnahme shifts (Aushilfe)**: bei POST/PATCH shifts genügt Mitgliedschaft in irgendeinem erlaubten Team des Anfragers (`isUserInAllowedTeams`) — Aushilfe aus einem anderen eigenen Team ist erlaubt, Schicht landet trotzdem im Ziel-Team; komplett fremde Nutzer weiter 403.
  - PATCH/DELETE/GET:id: Row-`teamId` muss in `getAllowedTeamIds` liegen, sonst 404 (IDOR).
  - `ShiftUpdate` erlaubt optionales `userId` (Assistenten-Wechsel, nur Massenbearbeitung) mit derselben Aushilfe-Regel (`isUserInAllowedTeams`) + Überschneidungsprüfung gegen den NEUEN Nutzer; Team der Schicht bleibt.
...
- **E2E-Code wird mitgeprüft**: `pnpm run typecheck` (bzw. `pnpm --filter @workspace/dienstplan run typecheck`) prüft neben `src/` auch `e2e/**/*` + `playwright.config.ts` via `tsconfig.e2e.json` — Typfehler in Specs/Helpern (z. B. fehlendes `await` auf async DB-Helfern) fallen sofort auf, nicht erst im Suite-Lauf.
- **Merge-Validierung** (registrierte Validation-Steps, laufen bei jedem Task-Abschluss/Merge): `typecheck` (`pnpm run typecheck`) + `e2e-api` (`pnpm --filter @workspace/dienstplan run test:e2e:api` = alle `*-api.spec.ts`, ~3 min). Rote API-Specs blockieren so den Merge, statt still zu driften.
