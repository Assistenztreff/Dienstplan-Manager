...
- **Durchgesetzte Gates** (403 `plan_limit_reached` / `plan_feature_required`):
  ...
  - `absenceTracking`: Frontend-Gate in `abwesenheiten.tsx` (Free: Upgrade-Hinweis statt „Resturlaub"-Karte) + Server-Gate auf `GET /contracts/:id/vacation-balance` (via Team-Owner-Plan). Assistenten nur die EIGENE Bilanz (fremder Vertrag → 404, auch team-intern). `AssistantVacationCard` (nur Nicht-Admins) bei 403 still ausgeblendet — kein Upsell an Assistenten. EINTRAGEN von Urlaub/Krankheit bleibt für alle Pläne frei; Rohdaten + `vacationDaysUsed`-Buchhaltung laufen plan-unabhängig.
  - `strictTimeTracking` (`PATCH /time-tracking/:id/confirm`): Bestätigen/Ablehnen nur Premium. `dashboard/summary` zählt Ist-Stunden plan-abhängig: bestätigte immer; „offene" zusätzlich in Teams mit Free-Eigentümer (beide Branches); abgelehnte nie. Warnung „offene Zeiteinträge" nur bei `warnings.timeTrackingConfirmable` (ein Premium-Eigentümer im Scope) — kein Upsell; Zähler/KPI bleiben korrekt. KEIN Auto-Bestätigen beim Upgrade; `uncountedPendingHours`/`-Entries` (pending in strikten Teams) → `UncountedPendingNotice` mit Link zur Zeiterfassung.
  - `calendarSync`: ICS-Export der FIX-Schichten (`GET /calendar-export`) + Abo-Feed `GET /calendar-feed/:token` (public; Token unique/nullable; Premium-Gate über Token-EIGENTÜMER, 404 bei unbekanntem/widerrufenem Token). `GET/POST /calendar-token` Premium (POST rotiert); `DELETE` bewusst OHNE Plan-Gate (Widerruf auch nach Downgrade). Assistenten via Arbeitgeber-Plan. `CalendarExportCard` (Assistenten) probt `GET /calendar-token` (403 = gesperrt).
- **Frontend-Gates** (`PLAN_FEATURE_MESSAGES`): PDF-Nachweis-Buttons, Einladen-Button + InviteDialog, Zeiterfassungs-Bestätigen, Kalender-Export-Card. Limit-Hinweise: `plan-limit-banner.tsx`.
- **Einfacher Monats-Export** (`basicExport`, auch Free): Button „Monats-PDF" im Dienstplan-Header für Admin UND Assistent. Rein aus `GET /shifts`: FIX-Dienste + Abwesenheiten (ganztägig, „1 Tag"), OHNE Zeiterfassung/Soll-Ist/Zuschläge. Respektiert Assistenten-Filter; eine Seite pro Assistent mit Einträgen.
