...
- **Serverseitige Durchsetzung (autoritativ, Task #205)**: Der Client ist nicht vertrauenswürdig — die Frontend-Gates sind reine UX. Der API-Server setzt die Limits jetzt verbindlich mit DERSELBEN Config durch. Helfer in `artifacts/api-server/src/lib/plan.ts`: `getUserPlan(userId)` (liest `users.plan` IMMER frisch aus der DB, analog `requireDienstleister` — manuelle Premium-Freischaltung wirkt sofort), `userHasFeature`, `userWithinLimit`, Middleware `requirePlanFeature(feature)`.
  ...
  - **maxTeams** in `routes/teams.ts` POST: zählt die vom Nutzer besessenen Teams (`teams.owner_id = session.userId`) und liefert 403 `{code:"plan_limit_reached", limit:"maxTeams"}`, wenn das Free-Limit (1) erreicht ist. **Bestandsschutz**: nur das Anlegen eines WEITEREN Teams über dem Limit wird gesperrt; vorhandene Teams (inkl. dem bei der Registrierung erzeugten „Standard-Team") bleiben unberührt. Da die Registrierung bereits ein Team anlegt, startet ein Free-Dienstleister direkt am Limit (1) → das zweite Team wird geblockt. Abgedeckt durch `e2e/dienstplan-free-team-limit-api.spec.ts` (frisch registriertes Free-Dienstleister-Konto, zweites Team → 403).
  ...
  - **maxAssistants** in `routes/users.ts` POST: greift nur, wenn `role === "assistant"` angelegt wird; gezählt werden die distinct Assistenz-Nutzer über ALLE Teams des **Ziel-Team-Eigentümers** (kontoweites Limit, `teams.owner_id`-Join), und maßgeblich ist der **Plan des Eigentümers**, nicht des ggf. anderen Anfragers (ein Premium-Member-Admin darf kein fremdes Free-Team-Limit überschreiten und umgekehrt). Liefert 403 `{code:"plan_limit_reached", limit:"maxAssistants"}` (Limit im Fehlertext dynamisch via `getUserLimit`) ab dem Free-Limit (6). Bestandsschutz: vorhandene Assistenten bleiben sichtbar/editierbar; nur das Anlegen über dem Limit ist gesperrt. **Genau ein** Enforcement-Block (nach `resolveWriteTeamId`, sobald das Ziel-Team bekannt ist) — der früher zusätzlich vorhandene, auf den Anfrager statt den Eigentümer bezogene Vorab-Block wurde als fehlerhafte Doppelung entfernt.
  - **historyMonths** in `routes/shifts.ts` POST UND PATCH: begrenzt das Voraus-Planen. Eine Schicht wird mit 403 `{code:"plan_limit_reached", limit:"historyMonths"}` abgelehnt, wenn ihr Kalendermonat mehr als `historyMonths` Monate nach dem aktuellen Monat liegt (Vergleich in ganzen Monaten). Free (1) erlaubt aktuellen + nächsten Monat. Zentral im Helfer `forwardPlanningBlocked` (eine Quelle für beide Pfade), der den Plan des **TEAM-EIGENTÜMERS** liest (`teams.owner_id`, nicht den ggf. abweichenden Anfrager — sonst könnte ein Member-Admin mit eigenem Premium-Plan das Free-Limit eines fremden Teams umgehen). **Vergangene Monate werden nie blockiert** (Backfill/Bestandsschutz). **PATCH wird nur geprüft, wenn `startTime` im Body ist** (Move-Forward-Bypass: sonst legt ein Free-Konto eine erlaubte Schicht an und schiebt sie per Edit weit in die Zukunft); reine Notiz-/Typ-Änderungen an bestehenden Schichten bleiben frei. Abgedeckt durch `e2e/dienstplan-free-plan-limits-api.spec.ts` (POST far-future + PATCH move-into-future).
  - **advancedPersonnelFile** in `routes/users.ts` POST + PATCH: Ein Free-Konto darf die Felder der erweiterten Personalakte (Lohn-/SV-Daten: `birthDate`, `socialSecurityNumber`, `taxId`, `taxClass`, `healthInsurance`, `iban`) NICHT setzen/ändern → 403 `{code:"plan_feature_required", feature:"advancedPersonnelFile"}`. Die einfache Personalakte (Name, E-Mail, Telefon, Adresse) bleibt frei. **Bestandsschutz**: Lesen ist nie gesperrt; PATCH vergleicht gegen den DB-Stand und blockt nur ECHTE Änderungen — unverändert zurückgesendete Bestandswerte (z.B. wenn das Formular alle Felder mitschickt) gehen durch.
  - **advancedAnalytics** in `routes/dashboard.ts`: `GET /dashboard/hours-balance` (Soll/Ist-Auswertung inkl. Zuschlags-Aufschlüsselung) ist mit `requirePlanFeature("advancedAnalytics")` gegated → Free erhält 403. Das ist zugleich die Datenquelle für den clientseitig erzeugten Lohn-/Stundennachweis-PDF-Export, also wird **payrollExport** transitiv mit-durchgesetzt. Bestandsschutz bleibt gewahrt: die ROHDATEN (Schichten, Zeiterfassung, Verträge) bleiben über ihre regulären Listen-Endpunkte sichtbar — gesperrt wird nur diese abgeleitete Premium-Auswertung; `GET /dashboard/summary` (Basis-KPIs) bleibt frei.
  - **Noch nicht serverseitig durchgesetzt** (reine Konfiguration/Frontend-UX): Die Feature-Flags `strictTimeTracking`, `calendarSync`, `caregiverLogin` sowie ein eigenständiger `payrollExport`-Endpunkt haben aktuell KEINE serverseitige Entsprechung (keine implementierten Endpunkte/Aktionen) und können daher nicht gegated werden, ohne Kernfunktionen (Login der Assistenzkräfte, Basis-Zeiterfassung) zu brechen — sie bleiben bewusst Konfiguration, bis es echte Endpunkte gibt. Beispiel-Frontend-Gates: 4. Schichtmodell (`einstellungen.tsx`), Massenbearbeitung „Mehrere bearbeiten" (`dienstplan.tsx`). Durchgesetzt: `maxShiftModels`, `maxAssistants`, `maxTeams`, `historyMonths`, `bulkEdit`, `advancedPersonnelFile`, `advancedAnalytics` (inkl. payrollExport-Quelle).
- **VERBINDLICHE REGEL — Bestandsschutz**: Free-Limits beschränken AUSSCHLIESSLICH das Anlegen von NEUEM (bzw. neue Aktionen). Bereits vorhandene Daten — Teams, Lohndaten, bereits geplante/vergangene Monate, vorhandene Schichtmodelle — dürfen NIEMALS ausgeblendet, gesperrt oder gelöscht werden, nur weil ein Konto auf dem Free-Plan ist. `isWithinLimit(user, limit, count)` prüft, ob ein WEITERER Eintrag erlaubt ist — es ist KEIN Anzeige-Filter. Konkret: `maxShiftModels` sperrt nur das Anlegen über dem Limit (vorhandene Modelle bleiben editierbar/löschbar), `maxAssistants`/`maxTeams` blockieren nur das Anlegen über dem Limit, `historyMonths` begrenzt nur die neue Planung/Vorwärts-Navigation (bereits erfasste Monate bleiben sichtbar), `advancedPersonnelFile`/`payrollExport` lassen bereits erfasste Lohndaten sichtbar. Aktuell sind alle vier Limits (`maxShiftModels`, `maxAssistants`, `maxTeams`, `historyMonths`) sowie die Features `bulkEdit`, `advancedPersonnelFile` und `advancedAnalytics` serverseitig durchgesetzt; die übrigen Feature-Flags sind noch reine Konfiguration ohne Wirkung (blenden also nichts aus). Details siehe Kommentarblock in `entitlements.ts`.
