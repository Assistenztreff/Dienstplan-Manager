...
- `pnpm --filter @workspace/dienstplan run test:e2e` — Playwright gegen **isolierten Test-Stack** (API 8099 + Vite 5199, DB `<dbname>_test`); Dev-DB unberührt. Beim Config-Load laufen automatisch `setup-test-db`, dann `verify-account-separation` (~20s, Fehlschlag bricht ab). Skips: `E2E_SKIP_SEPARATION_CHECK=1`, `E2E_SKIP_DB_SETUP=1`, `E2E_BASE_URL`. Test-Konten MÜSSEN dem Muster `e2e.*@dienstplan.test` folgen (`cleanup-test-accounts` räumt vor/nach jedem Lauf).
