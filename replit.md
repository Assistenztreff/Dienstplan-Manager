...
- `pnpm --filter @workspace/scripts run setup-superadmin -- <email> <passwort> [name]` — Betreiber-Konto (idempotent; alternativ Env `SUPERADMIN_*`; kein Default-Passwort, min. 8 Zeichen; bestehendes Konto wird befördert, Passwort bleibt)
- `pnpm --filter @workspace/scripts run setup-test-db` — Test-DB anlegen/aktualisieren (idempotent)
- `pnpm --filter @workspace/dienstplan run test:e2e` — Playwright gegen **isolierten Test-Stack** (API 8099 + Vite 5199, eigene DB `<dbname>_test`, via `setup-test-db` auto-provisioniert); Dev-DB wird nicht berührt. Override via `E2E_BASE_URL`. Test-Konten (`e2e.*@dienstplan.test`) räumt `cleanup-test-accounts` vor/nach jedem Lauf ab — Test-E-Mails MÜSSEN dieses Muster behalten.
...
- **Durchgesetzte Gates** (403 `plan_limit_reached` / `plan_feature_required`):
  ...
  - `absenceTracking`: NUR Frontend-Gate (kein Server-Gate, keine eigene API — Bilanz wird clientseitig aus frei zugänglichen Daten berechnet). Das EINTRAGEN von Urlaub/Krankheit bleibt für alle Pläne frei; Premium ist nur die Resturlaub-Zählung: `abwesenheiten.tsx` ersetzt bei Free den Inhalt der Karte „Resturlaub {Jahr}" durch den Upgrade-Hinweis. `vacationDaysUsed`-Buchhaltung läuft plan-unabhängig weiter (kein Datenverlust bei Upgrade).
