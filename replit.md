...
- **Serverseitige Durchsetzung (autoritativ, Task #205)**: Der Client ist nicht vertrauenswürdig — die Frontend-Gates sind reine UX. Der API-Server setzt die Limits jetzt verbindlich mit DERSELBEN Config durch. Helfer in `artifacts/api-server/src/lib/plan.ts`: `getUserPlan(userId)` (liest `users.plan` IMMER frisch aus der DB, analog `requireDienstleister` — manuelle Premium-Freischaltung wirkt sofort), `userHasFeature`, `userWithinLimit`, Middleware `requirePlanFeature(feature)`.
  ...
  - **historyMonths** in `routes/shifts.ts` POST (nur Anlegen, NICHT PATCH): begrenzt das Voraus-Planen. Eine Schicht wird mit 403 `{code:"plan_limit_reached", limit:"historyMonths"}` abgelehnt, wenn ihr Kalendermonat mehr als `historyMonths` Monate nach dem aktuellen Monat liegt (Vergleich in ganzen Monaten, UTC). Free (1) erlaubt aktuellen + nächsten Monat. **Vergangene Monate werden nie blockiert** (Backfill/Bestandsschutz), und PATCH bleibt ungated, damit bestehende Schichten editierbar bleiben.
